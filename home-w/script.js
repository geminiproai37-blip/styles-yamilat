...
export function createHeroSlider(backdropBaseUrl, items) {
  ...
  items.forEach((item, index) => {
    if (item && item.poster_path) {
      ...
      // Display release date and ranking stacked vertically
      const dateAndRankingContainer = document.createElement("div");
      dateAndRankingContainer.className =
        "flex flex-col items-start text-lg space-y-1"; // Changed to flex-col and items-start

      // Ranking
      if (item.vote_average) {
        const ranking = document.createElement("span");
        ranking.className = "flex items-center text-lg"; // Added text-lg for consistent sizing
