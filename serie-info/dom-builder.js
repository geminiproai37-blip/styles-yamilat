...
export async function buildMovieDetailPage(
  ...
  function updatePlayButtonState(
    ...
    console.log("Watched Episodes from localStorage:", watchedEpisodes);
    ...
    for (const sNum of availableSeasons) {
      ...
      console.log(`Season ${sNum} Episodes:`, episodesInSeason);
      ...
      for (const episode of episodesInSeason) {
        ...
        console.log(
          `Episode S${sNum}E${episode.episode_number}: isWatched = ${isWatched}`
        );

        if (isWatched) {
          isAnyEpisodeWatched = true;
        } else if (!nextUnwatchedEpisode) {
          nextUnwatchedEpisode = { ...episode, season_number: sNum };
          console.log("Found first unwatched episode:", nextUnwatchedEpisode);
        }
      }
    }

    console.log("isAnyEpisodeWatched:", isAnyEpisodeWatched);
    console.log("nextUnwatchedEpisode:", nextUnwatchedEpisode);

    if (isAnyEpisodeWatched && nextUnwatchedEpisode) {
      playButton.innerHTML = `<i class="fas fa-play mr-2"></i> Continuar viendo E${nextUnwatchedEpisode.episode_number}`;
      playButton.href =
        nextUnwatchedEpisode.url ||
        `go:${movie.id}/season/${nextUnwatchedEpisode.season_number}/episode/${nextUnwatchedEpisode.episode_number}`;
      console.log(
        "Setting button to 'Continuar viendo'",
        playButton.innerHTML,
        playButton.href
      );
    } else {
      const firstSeason = availableSeasons[0];
      const firstEpisode = localEpisodesDb[mediaIdStr][firstSeason][0];
      playButton.innerHTML = `<i class="fas fa-play mr-2"></i> Reproducir E${firstEpisode.episode_number}`;
      playButton.href =
        firstEpisode.url ||
        `go:${movie.id}/season/${firstSeason}/episode/${firstEpisode.episode_number}`;
      console.log(
        "Setting button to 'Reproducir'",
        playButton.innerHTML,
        playButton.href
      );
    }
  ...
  trailerButton.className =
    "bg-orange-500 text-white px-6 py-3 rounded-lg font-bold flex items-center justify-center transition-colors hover:bg-orange-600 text-base"; // Changed orange shades slightly
      ...
      if (episodesToDisplay && episodesToDisplay.length > 0) {
        episodesToDisplay.forEach((episode) => {
          ...
          markWatchedBtn.addEventListener("click", (event) => {
            // event.stopPropagation(); // Removed to allow navigation if desired
            toggleEpisodeWatched(markWatchedBtn);

            // Navigate to the episode's URL after marking as watched
            const mediaId = markWatchedBtn.dataset.mediaId;
            const seasonNumber = markWatchedBtn.dataset.seasonNumber;
            const episodeNumber = markWatchedBtn.dataset.episodeNumber;
            const episodeUrl =
              episode.url ||
              `go:${mediaId}/season/${seasonNumber}/episode/${episodeNumber}`;
