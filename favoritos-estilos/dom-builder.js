...
export function buildNavigationBar() {
  ...
  nav.innerHTML = `
    <button id="nav-home-btn" aria-label="Inicio"
      class="flex flex-col items-center text-gray-400 w-1/4 py-1 rounded-lg transition">
      <i class="fas fa-home text-lg md:text-xl mb-1"></i>
      <span class="text-xs font-semibold">Inicio</span>
    </button>
    <button id="nav-series-btn" aria-label="Series"
    ...
    <button id="nav-movies-btn" aria-label="Películas"
      class="flex flex-col items-center text-gray-400 hover:text-orange-500 w-1/4 py-1 rounded-lg transition">
      <i class="fas fa-film text-lg md:text-xl mb-1"></i>
      <span class="text-xs font-semibold">Películas</span>
    </button>
    <button id="nav-favorites-btn" aria-label="Favoritos"
  ...
  nav.querySelector("#nav-home-btn").addEventListener("click", () => {
    window.location.href = "go:home"; // Placeholder URL for Inicio
  });
  nav.querySelector("#nav-series-btn").addEventListener("click", () => {
    window.location.href = "go:series"; // Placeholder URL for Series
  });
  nav.querySelector("#nav-movies-btn").addEventListener("click", () => {
    window.location.href = "go:movies"; // Placeholder URL for Películas
  });
  nav.querySelector("#nav-favorites-btn").addEventListener("click", () => {
    window.location.hash = "go:favoritos"; // Favorites still uses hash for internal navigation
  });
...
export async function buildMovieDetailPage(
  ...
  mediaType,
  mediaId,
  seasonNumber, // Added seasonNumber as a parameter
  episodeNumber,
  ...
  if (trailer) {
    ...
    // Fallback if no trailer or backdrop
  ...
  playButton.href = `go:${movie.id}`; // Assuming 'go:id' is the play action
  ...
  if (
    ...
    const loadEpisodes = async (selectedSeasonNumber) => {
      ...
      console.log("loadEpisodes called for season:", selectedSeasonNumber);
      ...
      if (localEpisodeStubs.length > 0) {
        ...
        console.log(
          "Episodes to display (from local DB, merged with TMDB data):",
          episodesToDisplay
        );
      } else {
        ...
        episodesToDisplay.forEach((episode) => {
          ...
          episodeCard.innerHTML = `
            ...
            episode.name
              ...
              <a href="${
                episode.url ||
                `go:${movie.id}/season/${selectedSeasonNumber}/episode/${episode.episode_number}`
            ...
            <div class="p-3 flex-grow">
              <h4 class="font-bold text-base mb-1">${episode.name}</h4>
          ...
          // Create and append the mark-watched button separately
          const markWatchedBtn = document.createElement("button");
          markWatchedBtn.className =
            "mark-watched-btn mt-2 text-xs px-2 py-1 rounded-full transition-colors duration-200";
          markWatchedBtn.dataset.episodeId = episode.id;
          markWatchedBtn.dataset.mediaId = movie.id;
          markWatchedBtn.dataset.seasonNumber = selectedSeasonNumber;
          markWatchedBtn.dataset.episodeNumber = episode.episode_number;
          markWatchedBtn.innerHTML =
            '<i class="far fa-eye mr-1"></i> Marcar como visto';

          // Append the button to the episodeCard's content div (the one with class "p-3 flex-grow")
          const episodeCardContentDiv =
            episodeCard.querySelector(".p-3.flex-grow");
          if (episodeCardContentDiv) {
            episodeCardContentDiv.appendChild(markWatchedBtn);
          } else {
            console.error(
              "Episode card content div not found for appending mark-watched button."
            );
          }
          ...
          markWatchedBtn.addEventListener("click", (event) => {
            event.stopPropagation(); // Prevent the episode card link from being triggered
    ...
    function updateWatchedButtonState(button) {
      if (!button) {
        console.error("updateWatchedButtonState called with null button.");
        return;
      }
      if (!button.dataset) {
        console.error(
          "updateWatchedButtonState: button.dataset is null/undefined."
        );
        return;
      }
    ...
    // Helper function to toggle watched status
    function toggleEpisodeWatched(button) {
      if (!button) {
        console.error("toggleEpisodeWatched called with null button.");
        return;
      }
      const mediaId = button.dataset.mediaId;
      const seasonNumber = button.dataset.seasonNumber;
  ...
  // Construct the URL for the Facebook comments plugin without parameters
  const commentsUrl = window.location.href.split("?")[0].split("#")[0]; // Get base URL without hash or query params
...
export function buildFavoritesPage(imageBaseUrl) {
  const favoritesPageContainer = document.createElement("div");
  favoritesPageContainer.className =
    "w-full min-h-screen bg-gray-900 text-white max-w-4xl mx-auto p-4 pb-20"; // Added padding, and padding-bottom for nav

  // Function to create and show a custom confirmation modal
  const showConfirmationModal = (message, onConfirm) => {
    ...
    if (!modal) {
      ...
      modal.className =
        "fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[100] hidden";
      modal.innerHTML = `
        <div class="bg-gray-800 p-6 rounded-lg shadow-xl max-w-sm w-full text-center">
          ...
          <div class="flex justify-center space-x-4">
            <button id="modal-cancel-btn" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded transition-colors">
              Cancelar
            </button>
            <button id="modal-confirm-btn" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded transition-colors">
              Eliminar
            </button>
          </div>
        </div>
  ...
  // Function to render the favorites grid based on a filter
  const renderFavoritesGrid = (filterType) => {
    // Clear previous content except title and tabs
    const existingGrid =
      favoritesPageContainer.querySelector("#favorites-grid");
    if (existingGrid) {
      existingGrid.remove();
    }
    const existingNoFavoritesMessage = favoritesPageContainer.querySelector(
      "#no-favorites-message"
    );
    if (existingNoFavoritesMessage) {
      existingNoFavoritesMessage.remove();
    }
    ...
    if (filteredFavorites.length > 0) {
      ...
      favoritesGrid.className =
        "grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 mt-4"; // Smaller gap

      filteredFavorites.forEach((item) => {
        const favoriteCard = document.createElement("div");
        favoriteCard.className =
          "relative rounded-lg overflow-hidden shadow-lg transition-transform duration-200 hover:scale-105"; // Removed group, adjusted hover

        const posterLink = document.createElement("a");
        posterLink.href = `go:${item.id}`; // Link to the detail page
        posterLink.className = "block";

        const posterImage = document.createElement("img");
        posterImage.src = `${imageBaseUrl}${item.poster_path}`;
        posterImage.alt = item.title || item.name;
        posterImage.className = "w-full h-auto object-cover";
        posterLink.appendChild(posterImage);

        const deleteButton = document.createElement("button");
        deleteButton.className =
          "absolute top-1 right-1 bg-red-600 text-white text-xs p-1 rounded-full flex items-center justify-center w-6 h-6 transition-colors hover:bg-red-700"; // Always visible, smaller, adjusted position
        deleteButton.innerHTML = '<i class="fas fa-times"></i>';
        deleteButton.title = "Eliminar de favoritos";
        deleteButton.dataset.id = item.id;
        deleteButton.dataset.type = item.type;

        deleteButton.addEventListener("click", (event) => {
          console.log("Delete button clicked for item:", item); // Debugging line
          event.preventDefault();
          event.stopPropagation();

          // Capture item details from the button's dataset immediately
          const itemIdToRemove = parseInt(event.currentTarget.dataset.id);
          const itemTypeToRemove = event.currentTarget.dataset.type;
          ...
          showConfirmationModal(
            ...
            (confirmed) => {
              console.log("Confirmation modal result:", confirmed); // Debugging line
              if (confirmed) {
                let currentFavorites =
                  JSON.parse(localStorage.getItem("yamiLatFavorites")) || [];
                console.log("Favorites before deletion:", currentFavorites); // Debugging line
                console.log(
                  `Attempting to remove ID: ${itemIdToRemove}, Type: ${itemTypeToRemove}`
                ); // Debugging line

                currentFavorites = currentFavorites.filter(
                  (fav) =>
                    !(
                      Number(fav.id) === itemIdToRemove &&
                      fav.type === itemTypeToRemove
                    )
                );
                console.log(
                  "Favorites after deletion (filtered):",
                  currentFavorites
                ); // Debugging line
                ...
                console.log("Local storage updated."); // Debugging line

                // Re-render the current tab's content
                renderFavoritesGrid(currentActiveTab);
                console.log("Favorites grid re-rendered."); // Debugging line
              }
            }
          );
        });

        favoriteCard.appendChild(posterLink);
        favoriteCard.appendChild(deleteButton);
        favoritesGrid.appendChild(favoriteCard);
      });
      favoritesPageContainer.appendChild(favoritesGrid);
    } else {
      const noFavoritesMessage = document.createElement("p");
      noFavoritesMessage.id = "no-favorites-message";
      noFavoritesMessage.className =
        "text-center text-gray-400 text-base mt-10"; // Smaller text
      noFavoritesMessage.textContent = `No tienes ${
        filterType === "movie"
          ? "películas"
          : filterType === "tv"
          ? "series"
      ...
      favoritesPageContainer.appendChild(noFavoritesMessage);
    }
  };

  // "Misfavoritos" title
  const title = document.createElement("h2");
  title.className = "text-3xl font-bold text-center mb-4 mt-4 text-orange-500"; // Centered, bold, margin-bottom, margin-top, orange color
  title.textContent = "Mis Favoritos";
  favoritesPageContainer.appendChild(title);

  // Tabs for Movies and Series
  const tabsContainer = document.createElement("div");
  tabsContainer.className = "flex justify-center border-b border-gray-700 mb-4";
  favoritesPageContainer.appendChild(tabsContainer);

  const moviesTabButton = document.createElement("button");
  moviesTabButton.id = "movies-tab-btn";
  moviesTabButton.className =
    "flex-1 py-2 text-sm font-semibold text-gray-400 border-b-2 border-transparent hover:text-orange-500 hover:border-orange-500 focus:outline-none transition-colors duration-200";
  moviesTabButton.textContent = "Películas";
  tabsContainer.appendChild(moviesTabButton);

  const seriesTabButton = document.createElement("button");
  seriesTabButton.id = "series-tab-btn";
  seriesTabButton.className =
    "flex-1 py-2 text-sm font-semibold text-gray-400 border-b-2 border-transparent hover:text-orange-500 hover:border-orange-500 focus:outline-none transition-colors duration-200";
  seriesTabButton.textContent = "Series";
  tabsContainer.appendChild(seriesTabButton);

  let currentActiveTab = "all"; // Default to showing all, or "movie" if preferred

  const activateTab = (tabType) => {
    currentActiveTab = tabType;
    moviesTabButton.classList.remove("text-orange-500", "border-orange-500");
    moviesTabButton.classList.add("text-gray-400", "border-transparent");
    seriesTabButton.classList.remove("text-orange-500", "border-orange-500");
    seriesTabButton.classList.add("text-gray-400", "border-transparent");

    if (tabType === "movie") {
      moviesTabButton.classList.add("text-orange-500", "border-orange-500");
    } else if (tabType === "tv") {
      seriesTabButton.classList.add("text-orange-500", "border-orange-500");
    }
    renderFavoritesGrid(tabType);
  };

  moviesTabButton.addEventListener("click", () => activateTab("movie"));
  seriesTabButton.addEventListener("click", () => activateTab("tv"));

  // Initial render
  activateTab("movie"); // Start with movies tab active
