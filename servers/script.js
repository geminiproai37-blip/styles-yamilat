...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const backButtonHeader = document
    .querySelector("header .fa-arrow-left")
    .closest("a"); // Get the back button in the header
  ...
  async function loadPageContent() {
    // Fetch episode details from TMDB with a timeout
    try {
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("Timeout fetching episode data")),
          5000
        )
      );
      ...
      showMessage(
        "Error",
        "No se pudo cargar la información del episodio. Inténtalo de nuevo más tarde."
      );
    }
  ...
  castBtnGlobal.addEventListener("click", () => {
    if (currentServerUrl) {
      window.open(
        `https://www.webvideocaster.com/cast?url=${encodeURIComponent(
          currentServerUrl
        )}`,
        "_blank"
      );
