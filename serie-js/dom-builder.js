...
export function buildHeader(backButtonTargetId = null) {
...
export async function buildMovieDetailPage(
  ...
  function updatePlayButtonState(
    ...
    let lastWatchedEpisode = null; // New variable to store the last watched episode
    ...
    for (const sNum of availableSeasons) {
      ...
      for (const episode of episodesInSeason) {
        ...
        if (isWatched) {
          ...
          lastWatchedEpisode = { ...episode, season_number: sNum }; // Update last watched episode
    ...
    if (isAnyEpisodeWatched) {
      // If there's a next unwatched episode, link to it, but show the last watched episode number in the text
      if (nextUnwatchedEpisode) {
        playButton.innerHTML = `<i class="fas fa-play mr-2"></i> Continuar viendo E${lastWatchedEpisode.episode_number}`;
        playButton.href =
        ...
        // All episodes are watched, link to the last watched episode
        playButton.innerHTML = `<i class="fas fa-play mr-2"></i> Reproducir E${lastWatchedEpisode.episode_number}`;
        playButton.href =
          lastWatchedEpisode.url ||
          `go:${movie.id}/season/${lastWatchedEpisode.season_number}/episode/${lastWatchedEpisode.episode_number}`;
      }
    } else {
      // No episodes watched, default to the first episode
        ...
        episodesToDisplay.forEach((episode) => {
          ...
          episodeCard.innerHTML = `
            ...
            <div class="p-3 flex-grow">
              <h4 class="font-bold text-base mb-1">E${
