...
export function buildMainContent() {
  ...
  main.innerHTML = `
    ...
    <!-- Movies Section -->
    <div id="movies-section" class="p-5">
      <h2 class="text-xl font-bold mb-6 text-center text-orange-500 sticky top-0 bg-gray-900/80 backdrop-blur-sm z-40 py-4">
        <i class="fas fa-film text-lg mr-2"></i>Todas las Películas
      </h2>
      <!-- Movie grid content will be loaded here by script.js -->
    </div>
  `;
  return main;
}
...
export function createMovieGrid(imageBaseUrl, movies) {
  const movieGrid = document.createElement("div");
  movieGrid.id = "movie-grid";
  movieGrid.className = "grid grid-cols-3 gap-4"; // 3 columns grid

  movies.forEach((movie) => {
    if (movie && movie.poster_path) {
      const movieCard = document.createElement("a");
      movieCard.href = `go:${movie.id}`;
      movieCard.className = "flex flex-col items-center text-center";
