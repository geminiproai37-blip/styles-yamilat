...
export async function buildMovieDetailPage(
  ...
  externalPlayerButton.addEventListener("click", () => {
    if (currentServerUrl) {
      ...
      const absoluteUrl = new URL(currentServerUrl, window.location.origin)
        .href;
      openWithAndroidChooser(absoluteUrl); // Directly open the chooser
    } else {
    ...
    localServersDb[lang].servers.forEach((server) => {
      ...
      serverButton.addEventListener("click", () => {
        ...
        if (isDirectVideoLink) {
          ...
          // Attempt to load video and initialize Plyr
          try {
            const videoElement = document.createElement("video");
            videoElement.controls = true;
            videoElement.crossOrigin = "anonymous";
            videoElement.playsInline = true;
            videoElement.className = "plyr-video w-full h-full";
            videoElement.innerHTML = `<source src="${server.url}" type="video/mp4">`;
            ...
            player.on("error", () => {
              console.error("Plyr error: Failed to load video.", server.url);
              iframeContainer.innerHTML = `
                <div class="flex flex-col items-center justify-center h-full bg-gray-900 text-red-400 p-4">
                  <i class="fas fa-exclamation-circle text-4xl mb-2"></i>
                  <p class="text-center">Error al cargar el video. El enlace podría estar roto o no disponible.</p>
                </div>
              `;
            });
          } catch (error) {
            ...
            iframeContainer.innerHTML = `
              <div class="flex flex-col items-center justify-center h-full bg-gray-900 text-red-400 p-4">
                <i class="fas fa-exclamation-circle text-4xl mb-2"></i>
                <p class="text-center">Error al preparar el reproductor de video.</p>
              </div>
            `;
          }
        } else {
          // Set sandbox attribute based on server's blockAds property
          if (server.blockAds) {
            ...
            iframeElement.setAttribute(
              "sandbox",
              "allow-forms allow-modals allow-orientation-lock allow-pointer-lock allow-popups allow-popups-to-escape-sandbox allow-presentation allow-same-origin allow-scripts allow-top-navigation-by-user-activation allow-top-navigation"
            );
          }
  ...
  // Add sandbox attribute to restrict iframe capabilities, specifically omitting 'allow-downloads'
  iframeElement.setAttribute(
    "sandbox",
    "allow-forms allow-modals allow-orientation-lock allow-pointer-lock allow-popups allow-popups-to-escape-sandbox allow-presentation allow-same-origin allow-scripts allow-top-navigation-by-user-activation allow-top-navigation"
  ); // Added allow-top-navigation
  iframeContainer.appendChild(iframeElement);
