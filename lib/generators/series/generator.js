...
<html lang="es">
  <head>
    ...
    <link
      rel="stylesheet"
      href="https://cdn.jsdelivr.net/gh/geminiproai37-blip/styles-yamilat@main/series-estilos/style.css"
    />
  </head>
  <body class="bg-gray-900 text-white">
    ...
    <script>
      const seriesData = ${seriesDataString}
    </script>
    <script
      type="module"
      src="https://cdn.jsdelivr.net/gh/geminiproai37-blip/styles-yamilat@main/series-estilos/main.js"
    ></script>
  </body>
</html>
`;
  ...
  const renderMediaList = async () => {
    ...
    sectionTitleDisplay.textContent = contentData[0].title || "Series de Anime";
  ...
  confirmAddItemBtn.addEventListener("click", () => {
    ...
    addItemToContentData(seriesId, "tv", sectionTitle, isHero);
  ...
  confirmEditItemBtn.addEventListener("click", () => {
    if (itemToEditIndex !== null) {
      ...
      updateItemInContentData(itemToEditIndex, seriesId, "tv");
      editItemModal.classList.add("hidden");
    }
  });
  ...
  const loadHtmlIntoGenerator = (htmlContent) => {
    ...
    scriptTags.forEach((script) => {
      if (script.textContent.includes("const seriesData =")) {
        try {
          const dataStringMatch = script.textContent.match(
            /const\s+seriesData\s*=\s*(\[[^]*?\]);?/s // Use [^]*? to match any character including newlines, non-greedily
          );
          if (dataStringMatch && dataStringMatch[1]) {
            let jsonString = dataStringMatch[1];
            console.log("Extracted raw seriesData string:", jsonString);

            // 1. Remove comments (single-line comments)
            jsonString = jsonString.replace(/\/\/.*$/gm, "");
            console.log("seriesData after removing comments:", jsonString);

            // 2. Add missing commas between objects that are on separate lines
            jsonString = jsonString.replace(/}\s*\n\s*{/g, "},\n      {");
            console.log("seriesData after adding missing commas:", jsonString);
            ...
            console.log("seriesData after quoting keys:", jsonString);
            ...
            console.log(
              "seriesData after removing trailing commas:",
              jsonString
            );
            ...
            try {
              ...
              console.error(
                "Error parsing seriesData from loaded HTML:",
                error,
                "Processed string:",
                jsonString
              );
          ...
          console.error("Error parsing seriesData from loaded HTML:", error);
        }
      }
    });
    ...
    if (loadedItems) {
      ...
      alert(
        "El HTML proporcionado no contiene los datos de 'seriesData' necesarios para cargar en el generador."
      );
    }
  };
});
