  ...
  try {
    ...
    for (const lang of languagePriority) {
      ...
      if (type === "tv" && season_number !== null && episode_number !== null) {
        url = `${apiBaseUrl}/${type}/${id}/season/${season_number}/episode/${episode_number}?api_key=${TMDB_API_KEY}&language=${lang}`;
      } else {
        url = `${apiBaseUrl}/${type}/${id}?api_key=${TMDB_API_KEY}&language=${lang}`;
      }
    ...
    if (
      ...
      let url;
      if (type === "tv" && season_number !== null && episode_number !== null) {
        url = `${apiBaseUrl}/${type}/${id}/season/${season_number}/episode/${episode_number}?api_key=${TMDB_API_KEY}`;
      } else {
        url = `${apiBaseUrl}/${type}/${id}?api_key=${TMDB_API_KEY}`;
      }
      const detailsResponse = await fetch(url);
      if (detailsResponse.ok) {
        detailsData = await detailsResponse.json();
    ...
    try {
      ...
      if (type === "tv" && season_number !== null && episode_number !== null) {
        ...
        imagesUrl = `${apiBaseUrl}/${type}/${id}/season/${season_number}/episode/${episode_number}/images?api_key=${TMDB_API_KEY}&include_image_language=es-MX,es-ES,en,ja,null`;
      } else {
        // Fetch images for TV series/movies
        imagesUrl = `${apiBaseUrl}/${type}/${id}/images?api_key=${TMDB_API_KEY}&include_image_language=es-MX,es-ES,en,ja,null`;
      }
