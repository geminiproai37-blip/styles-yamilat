...
export function buildHeader() {
  ...
  header.querySelector("#search-btn").addEventListener("click", () => {
    window.location.href = "http://action_search"; // Placeholder for search action
  });
...
export async function buildMovieDetailPage(
  ...
  // Trailer or 16:9 Image (above logo)
  ...
  if (trailer) {
    ...
    trailerContainer.className =
      "w-full aspect-video mb-4 rounded-lg overflow-hidden shadow-lg";
    ...
    contentWrapper.appendChild(trailerContainer);
  } else if (movie.backdrop_path) {
    const smallImageContainer = document.createElement("div");
    smallImageContainer.className =
      "w-full aspect-video mb-4 rounded-lg overflow-hidden shadow-lg"; // 16:9 aspect ratio
    smallImageContainer.style.backgroundImage = `url(${backdropBaseUrl}${movie.backdrop_path})`;
    smallImageContainer.style.backgroundSize = "cover";
    smallImageContainer.style.backgroundPosition = "center";
    contentWrapper.appendChild(smallImageContainer);
  }

  // Movie Logo/Title and Categories
  ...
  if (movie.logo_path) {
    ...
    logo.className = "w-1/2 md:w-1/4 lg:w-1/5 mb-3 md:mb-0 drop-shadow-lg"; // Made logo smaller
    titleAndGenresContainer.appendChild(logo);
  } else {
    const title = document.createElement("h1");
    title.className =
      "text-3xl md:text-4xl lg:text-5xl font-bold mb-3 md:mb-0 drop-shadow-lg"; // Made title smaller
    title.textContent = movie.title || movie.name;
    titleAndGenresContainer.appendChild(title);
  }

  // Categories/Genres - Moved next to logo/title
  if (movie.genres && movie.genres.length > 0) {
    const genresContainer = document.createElement("div");
    genresContainer.className = "flex flex-wrap gap-2 mt-2 md:mt-0 md:ml-4"; // Adjusted gap and margin for alignment
    ...
    titleAndGenresContainer.appendChild(genresContainer);
  }

  // Movie Metadata (Year, Age Category, Seasons)
  const metadataContainer = document.createElement("div");
  metadataContainer.className =
    "flex items-center space-x-4 text-gray-400 text-sm mb-4";
  contentWrapper.appendChild(metadataContainer);

  if (movie.release_date) {
    const year = document.createElement("span");
    year.textContent = movie.release_date.substring(0, 4);
    metadataContainer.appendChild(year);
  }

  // Placeholder for Age Category (MPAA rating) - requires fetching content ratings
  // For now, a static placeholder or fetch logic needs to be added
  const ageRating = document.createElement("span");
  ageRating.className = "border border-gray-600 px-2 py-0.5 rounded-md text-xs";
  ageRating.textContent = movie.adult ? "+18" : "TP"; // Simple placeholder
  metadataContainer.appendChild(ageRating);

  if (movie.number_of_seasons) {
    // Only for TV shows
    const seasons = document.createElement("span");
    seasons.textContent = `${movie.number_of_seasons} Temporada${
      movie.number_of_seasons > 1 ? "s" : ""
    }`;
    metadataContainer.appendChild(seasons);
  }
  ...
  // Seasons and Episodes Section (always visible if applicable)
  if (movie.number_of_seasons) {
    ...
    if (localEpisodesDb[mediaIdStr]) {
      ...
      // Fallback to TMDB data if no local data for this mediaId
    ...
    const loadEpisodes = async (selectedSeasonNumber) => {
      ...
      // Fetch details for the entire season from TMDB
      const seasonDetails = await fetchTvSeasonDetails(
        apiBaseUrl,
        movie.id,
        selectedSeasonNumber
      );
      console.log("TMDB season details:", seasonDetails);
      ...
      if (localEpisodeStubs.length > 0) {
        // Fetch full details for each local episode stub
        const episodePromises = localEpisodeStubs.map(async (localEp) => {
          ...
          return fullEpisodeDetails
            ? { ...fullEpisodeDetails, url: localEp.url }
            : {
        ...
        console.log(
          "No local episodes found for season:",
          selectedSeasonNumber,
          ". No episodes will be displayed."
        );
      }
      ...
      if (episodesToDisplay && episodesToDisplay.length > 0) {
        episodesToDisplay.forEach((episode) => {
          ...
          episodeCard.className =
            "flex items-center bg-gray-800 rounded-lg overflow-hidden shadow-md relative"; // Added relative for positioning
